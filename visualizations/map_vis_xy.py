...
def draw_map_without_lanelet_xy(filename, axes, lat_origin, lon_origin):

    assert isinstance(axes, matplotlib.axes.Axes)

    axes.set_aspect('equal', adjustable='box')
    axes.patch.set_facecolor('white') # lightgrey

    projector = LL2XYProjector(lat_origin, lon_origin)

    e = xml.parse(filename).getroot()
    # print('e', e.findall('way'))
    ...
    for way in e.findall('way'):
        ...
        if way_type is None:
            ...
            if way_subtype == "dashed":
                type_dict = dict(color="grey", linewidth=1, zorder=10, dashes=[10, 10])
            else:
                type_dict = dict(color="grey", linewidth=1, zorder=10)
        elif way_type == "line_thick":
            way_subtype = get_subtype(way)
            if way_subtype == "dashed":
                type_dict = dict(color="grey", linewidth=2, zorder=10, dashes=[10, 10])
            else:
                type_dict = dict(color="grey", linewidth=2, zorder=10)
            ...
            type_dict = dict(color="grey", linewidth=3, zorder=10)
        elif way_type == "virtual":
            type_dict = dict(color="white", linewidth=1, zorder=10, dashes=[2, 5]) # blue
